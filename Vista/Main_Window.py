...
def initializeModel(model):
    ...
    model.setHeaderData(1, QtCore.Qt.Horizontal, "Year")
    model.setHeaderData(2, QtCore.Qt.Horizontal, "Month")
    model.setHeaderData(3, QtCore.Qt.Horizontal, "Day")
    model.setHeaderData(4, QtCore.Qt.Horizontal, "Fecha entrada")
    model.setHeaderData(5, QtCore.Qt.Horizontal, "Fecha salida")

def initializeModelFichada(modelFichada):
    modelFichada.setTable('BusquedaF')
    modelFichada.setEditStrategy(QtSql.QSqlTableModel.OnFieldChange)
    modelFichada.select()
    modelFichada.setHeaderData(0, QtCore.Qt.Horizontal, "ID")
    modelFichada.setHeaderData(1, QtCore.Qt.Horizontal, "Year")
    modelFichada.setHeaderData(2, QtCore.Qt.Horizontal, "Month")
    modelFichada.setHeaderData(3, QtCore.Qt.Horizontal, "Day")
    modelFichada.setHeaderData(4, QtCore.Qt.Horizontal, "Fecha entrada")
    modelFichada.setHeaderData(5, QtCore.Qt.Horizontal, "Fecha salida")
...
class MainWindow(QtGui.QWidget):
    ...
    def init_ui(self):
        ...
        button_filtrarRegistros = QtGui.QPushButton('Filtrar Registro')
        button_filtrarRegistros.clicked.connect(self.controlador.show_vv)
        layout_grid_sv.addWidget(button_filtrarRegistros)
        ...
        self.comboBox.addItems(["year", "month", "day"])
        self.displayDesde = QtGui.QLineEdit('Desde')
        self.displayHasta = QtGui.QLineEdit('Hasta')
        button_submit_vlado = QtGui.QPushButton('Submit')
        button_submit_vlado.clicked.connect(lambda: self.controlador.busqueda())
        button_atras_vlado = QtGui.QPushButton('Atras')
        button_atras_vlado.clicked.connect(self.controlador.atrasBusqueda)
        initializeModelFichada(self.modelFichada)
        viewFichada = QtGui.QTableView()
        viewFichada.setModel(self.modelFichada)
        view.setWindowTitle("Fichadas")
        layout_grid_vv = QtGui.QGridLayout()
        layout_grid_vv.addWidget(self.comboBox)
        layout_grid_vv.addWidget(self.displayDesde)
        layout_grid_vv.addWidget(self.displayHasta)
        layout_grid_vv.addWidget(button_submit_vlado)
        layout_grid_vv.addWidget(button_atras_vlado)
        layout_grid_vv.addWidget(viewFichada)
        self.box_vv = QtGui.QGroupBox()
        self.box_vv.setLayout(layout_grid_vv)
        self.box_vv.setHidden(True)
        ...
        self.main_grid.addWidget(self.box_vv, 1, 1, 1, 1)
...
timer.timeout.connect(lambda: initializeModelFichada(window.modelFichada))
timer.start(500)
sys.exit(app.exec_())
